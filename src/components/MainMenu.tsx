...
import { ProfileManager, Profile } from './ProfileManager';
import { 
  Dialog, 
  DialogContent, 
  DialogHeader, 
...
export const MainMenu: React.FC<MainMenuProps> = ({ onStartGame }) => {
  ...
  useEffect(() => {
    const profilesJson = localStorage.getItem('mtg-profiles');
    if (profilesJson) {
      try {
        ...
        const lastProfileId = localStorage.getItem('mtg-last-profile-id');
        if (lastProfileId) {
  ...
  const handleSelectProfile = (profile: Profile) => {
    if (currentEditingIndex !== null) {
      ...
      // If this is player 1, save as last used profile
      if (currentEditingIndex === 0) {
        localStorage.setItem('mtg-last-profile-id', profile.id);
      }
  ...
  // Handle profile creation from within MainMenu
