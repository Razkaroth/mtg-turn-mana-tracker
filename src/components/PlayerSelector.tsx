          ...
          >
            <div className="p-3 grid grid-cols-4 gap-2 max-w-sm mx-auto">
              {players.map((player, index) => (
                <Button
                  key={player.id}
                  variant={displayedPlayer === index ? "default" : "outline"}
                  size="sm"
                  className={`h-auto py-2 flex-col items-center justify-center ${
                    activePlayer === index ? "border-primary/60" : ""
                  }`}
                  onClick={() => handleSelectPlayer(index)}
                >
                  <div className="flex flex-col items-center">
                    <div className={`h-8 w-8 rounded-full ${
                      activePlayer === index ? "bg-primary" : "bg-muted"
                    } flex items-center justify-center mb-1`}>
                      <User className={`h-4 w-4 ${
                        activePlayer === index ? "text-primary-foreground" : "text-muted-foreground"
                      }`} />
                    </div>
